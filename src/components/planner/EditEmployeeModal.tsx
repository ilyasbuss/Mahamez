...
const EditEmployeeModal: React.FC<EditEmployeeModalProps> = ({ isOpen, employee, skillGroups, redaktionen, allEmployees, onClose, onSave, onDelete }) => {
    ...
    const generateEmailFromName = (name: string): string => {
        ...
        let email = `${parts[0]}.${parts[parts.length - 1]}`.toLowerCase();
        email = email.replace(/ä/g, 'ae').replace(/ö/g, 'oe').replace(/ü/g, 'ue').replace(/ß/g, 'ss');
        // Remove other special characters if any, but user only specified umlauts
        return `${email}@swr.de`;
    };

    const handleNameChange = (newName: string) => {
        const updates: Partial<Employee> = { name: newName };

        // Auto-generate email if it's currently empty or was previously auto-generated
        const currentEmail = editingEmployee.email || '';
        const wasAutoGenerated = currentEmail === '' || currentEmail === generateEmailFromName(editingEmployee.name);

        if (wasAutoGenerated) {
            updates.email = generateEmailFromName(newName);
        }

        setEditingEmployee({ ...editingEmployee, ...updates });
    };
                ...
                <form onSubmit={handleSubmit} className="p-5">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        <div className="space-y-4">
                            <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Name</label><input type="text" required value={editingEmployee.name} onChange={(e) => handleNameChange(e.target.value)} className="w-full border rounded-xl px-3 py-2 outline-none focus:border-[#4B2C82] bg-slate-50 font-medium text-sm" /></div>
                                    ...
                                    {redaktionen.map(dept => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`px-3 py-1.5 rounded-xl border text-[11px] font-bold transition-all ${isSelected ? 'border-[#4B2C82] bg-purple-50 text-[#4B2C82]' : 'border-slate-100 bg-white text-slate-400 hover:bg-slate-50'}`}
                                            >
                                                {dept}
                                            </button>
                                        );
